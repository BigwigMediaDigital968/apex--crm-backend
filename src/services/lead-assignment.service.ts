...
export const assignLead = async ({
  ...
  try {
    const lead = await Lead.findOne({ _id: leadId, isDeleted: false }).session(session);
    if (!lead) {
      throw new AppError("Lead not found", 404, "LEAD_NOT_FOUND");
    }
    ...
    // Validate employee branch match ONLY if actor is NOT HEAD
    if (!isHead) {
      const employeeBelongsToBranch = employee.branches.some(
        (branch) => branch.toString() === lead.branch.toString()
      );
...
export const assignLeadsBulk = async ({
  ...
  try {
    ...
    const leads = await Lead.find({ _id: { $in: validLeadIds }, isDeleted: false }).session(session);
    if (leads.length === 0) {
    ...
    await LeadAssignmentHistory.insertMany(historyDocs, { session });
